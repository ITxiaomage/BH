...
def get_kxyj_news(url=r'http://ypt.cnki.net/znapp/ScienceAPI/GetKXHotList?keyWord=&pageIndex=1&pageSize=10',
    ...
    for one_news in news_data:
        ...
        result_list.append(package_data_dict(
            title=one_news['title'],
            url=None,
            img=None,
            content=content,
            date=one_news['publishDate'],
            source=one_news['chkm'],
            label=1))
    return result_list


# 专家观点
def get_kxyj_exp_opi(url=r'http://ypt.cnki.net/znapp/ScienceAPI/GetNewsList?keyWord=&pageIndex=1&pageSize=50',
                     base_url=r'http://ypt.cnki.net/znapp/ScienceAPI/GetNewsDetail?id='):
    r = requests.get(url=url).text
    r_dict = json.loads(r)
    news_data = r_dict['resultModel']['data']
    result_list = []
    for one_news in news_data:
        ...
        result_list.append(package_data_dict(title=one_news['title'],
                                             url=response['resultModel']['data']['spiderUrl'],
                                             img=one_news['coverImage'],
                                             content=response['resultModel']['data']['content'],
                                             date=one_news['publishDate'].split(' ')[0].replace('/', '-'),
                                             source=one_news['resource'],
                                             label=2))
    return result_list
...
def get_rmw_data_dict(news_url=None, news_content=None, news_title=None, news_img=None, news_time=None,
                      news_source=None, base_url=None):
    soup = rm_spider_head(news_url)
    try:
        # 标题失败
        try:
    ...
    return package_data_dict(title=news_title, url=news_url, img=news_img, content=str(news_content), date=news_time,
                             source=news_source, label=3)
...
def get_kx_data(browser, url, base_url, label):
    ...
    if info_list_len:
        for i in range(1, info_list_len):
            news_url = browser.find_element_by_xpath(
                '//div[@class="bt-mod-wzpb-02"]/ul/li[{0}]//p/a'.format(i)).get_attribute('href')
...
def get_kexie_news_data_list():
    ...
    tzgc_url = r'http://www.cast.org.cn/col/col457/index.html'
    base_url = r'http://www.cast.org.cn'
    result_list.extend(get_kx_news_list(tt_url, base_url, 1))
    result_list.extend(get_kx_news_list(yw_url, base_url, 1))
    result_list.extend(get_kx_news_list(tzgc_url, base_url, 3))
    return result_list
...
def get_kx_news_list(url, base_url, label):
    ...
    for i in range(1, repalce_list_len):
        ...
        result_list.append(
            package_data_dict(title=title, url=news_url, img=news_img, content=news_content, date=news_time,
                              source='中国科协', label=label))
    return result_list
...
def TF_IDF(content, n=MAX_KEYWORDS):
    ...
    temp_list = jieba.analyse.extract_tags(con, topK=10, withWeight=False,
